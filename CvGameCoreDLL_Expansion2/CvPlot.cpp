...
bool CvPlot::IsAllowsSailLand() const
{
	...
	if (eImprovement != NO_IMPROVEMENT)
	{
		CvImprovementEntry* pkEntry = GC.getImprovementInfo(eImprovement);
		if (pkEntry)
			if (pkEntry->IsAllowsSailLand() == true)
			{
				return pkEntry->IsAllowsSailLand();
			}
			else if (pkEntry->IsMakesPassable() == true)
			{
				return pkEntry->IsMakesPassable();
			}
	}
	return m_bIsAllowsSailLand;
}
//	--------------------------------------------------------------------------------
void CvPlot::SetAllowsSailLand(bool AllowsSailLand)
{
	m_bIsAllowsSailLand = AllowsSailLand;
}
//	--------------------------------------------------------------------------------
...
void CvPlot::read(FDataStream& kStream)
{
	...
	kStream >> bitPackWorkaround;
	m_bIsAllowsSailLand = bitPackWorkaround;
...
bool CvPlot::canTrain(UnitTypes eUnit, bool, bool) const
{
	...
	{	
		...
		if(thisUnitDomain == DOMAIN_SEA)
		{	
			if (getPlotCity()->isAllowsWaterProduction()) {
				return true;
			}
