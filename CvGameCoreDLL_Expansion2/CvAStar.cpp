...
int WaterRouteValid(CvAStarNode* parent, CvAStarNode* node, int data, const void* pointer, CvAStar* finder)
{
	...
	if(pNewPlot->isWater() || pNewPlot->IsAllowsSailLand())
	{
		return TRUE;
	}
...
int TradeRouteWaterPathCost(CvAStarNode* parent, CvAStarNode* node, int data, const void* pointer, CvAStar* finder)
{
	...
	if (!pToPlot->isCity())
	{
		...
		if (!pToPlot->isWater() && !pToPlot->IsAllowsSailLand())
		{
			iCost += 1000;
		}
		else
		{
			if (!pToPlot->IsAllowsSailLand() && pToPlot->getTerrainType() != (TerrainTypes) GC.getSHALLOW_WATER_TERRAIN())	// Quicker isShallowWater test, since we already know the plot is water
			{
