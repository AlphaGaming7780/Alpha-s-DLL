...
bool CvUnit::canEnterTerrain(const CvPlot& enterPlot, byte bMoveFlags) const
{
	...
	bool bAllowsSailLand = enterPlot.IsAllowsSailLand();
	...
	{
	case DOMAIN_SEA:
		if(!enterPlot.isWater() && !canMoveAllTerrain() && !bAllowsSailLand)
		{
			...
			if(canMoveInto(*pLoopPlot))
			{
				if(pLoopPlot->getNumFriendlyUnitsOfType(this) < GC.getPLOT_UNIT_LIMIT())
				{
					...
					if(canEnterTerritory(pLoopPlot->getTeam()) && (isBarbarian() || !isEnemy(pLoopPlot->getTeam(), pLoopPlot)) && !pLoopPlot->isMountain())
					{
						...
						{
							if(getDomainType() != DOMAIN_SEA || (pLoopPlot->isFriendlyCity(*this, true) && pLoopPlot->isCoastalLand()) || pLoopPlot->isWater() || pLoopPlot->IsAllowsSailLand())
							{
...
bool CvUnit::PlotValid(CvPlot* pPlot) const
{
	...
	{
	case DOMAIN_SEA:
		if(pPlot->isWater() || canMoveAllTerrain() || pPlot->IsAllowsSailLand())
		{
			return true;
		}
