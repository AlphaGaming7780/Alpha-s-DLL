...
void CvUnitMovement::GetCostsForMove(const CvUnit* pUnit, const CvPlot* pFromPlot, const CvPlot* pToPlot, int iBaseMoves, int& iRegularCost, int& iRouteCost, int& iRouteFlatCost)
{
	...
	if(bIgnoreTerrainCost || (bFasterAlongRiver && pToPlot->isRiver()) || (bFasterInHills && pToPlot->isHills()) || (pUnit->getDomainType() == DOMAIN_SEA && pToPlot->IsAllowsSailLand()))
	{
		iRegularCost = 1;
	}
	else
	...
	{
		if(!(bIgnoreTerrainCost || bFasterAlongRiver || (pUnit->getDomainType() == DOMAIN_SEA && pToPlot->IsAllowsSailLand())) && bRiverCrossing)
		{
